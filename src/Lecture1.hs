...
lowerAndGreater n list = go 0 0 list
    where 
        ...
        go bigger smaller l = 
            if null l then
               show n ++ " is greater than " ++ show smaller ++ " elements and lower than " ++ show bigger ++ " elements" 
            else if head l > n then
                go (bigger + 1) smaller (tail l)
            else if head l < n then
                go bigger (smaller + 1) (tail l)
            else go bigger smaller (tail l)
